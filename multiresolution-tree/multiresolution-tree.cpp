...
BuildTreeIntPass2(std::vector<particle_int>& Particles, i64 Begin, i64 End, const grid_int& Grid, split_type Split, i8 Depth) {
  ...
  N = MIN(N, CellCountRight);
  if (N <= Threshold) {
    ArithmeticEncode(u32(N), u32(P), ProbCount[Depth][N][N], ProbCount[Depth][N], &Coder);
...
#endif
    if (Split == SpatialSplit)
      Left = BuildTreeIntDFSBlockBased(Particles, Begin, Mid, GridLeft, NextSplit, ResLvl, Depth+1);
    else if (Split == ResolutionSplit)
      Left = BuildTreeIntDFSBlockBased(Particles, Begin, Mid, GridLeft, NextSplit, ResLvl+1, Depth+1);
  }
  tree* Right = nullptr;
  if (Mid+1==End && CellCountRight==1) {
    auto G = GridRight;
...
#endif
    if (Split == SpatialSplit)
      Right = BuildTreeIntDFSBlockBased(Particles, Mid, End, GridRight, NextSplit, ResLvl, Depth+1);
    else if (Split == ResolutionSplit)
      Right = BuildTreeIntDFSBlockBased(Particles, Mid, End, GridRight, NextSplit, ResLvl+1, Depth+1);
  }
...
DecodeIntAdaptiveBFSPhase(std::queue<q_item_int>& Queue, DynamicHeap<heap_data, f32>& Heap, std::vector<stack>& Stacks) {
  ...
  while (!Queue.empty()) {
    ...
    if (InTheCut) {
      ...
      f64 Mean = f64(N) / 2; // mean
      f64 StdDev = sqrt(f64(N)) / 2; // standard deviation
      P = DecodeRange(Mean, StdDev, f64(0), f64(N), CdfTable, &Stream->Stream, &Stream->Coder);
#else
      P = DecodeCenteredMinimal(u32(N+1), &Stream->Stream);
#endif
      if (Flip) { P = CellCountLeft-P; N = CellCount-N; }
      Mid = P + Q.Begin;
...
#endif
    if (Split == SpatialSplit)
      NParticlesLeft += DecodeTreeIntDFSBlockBased(Begin, Mid, GridLeft, NextSplit, ResLvl, Depth+1);
    else if (Split == ResolutionSplit)
      NParticlesLeft += DecodeTreeIntDFSBlockBased(Begin, Mid, GridLeft, NextSplit, ResLvl+1, Depth+1);
  }
...
#endif
    if (Split == SpatialSplit)
      NParticlesRight += DecodeTreeIntDFSBlockBased(Mid, End, GridRight, NextSplit, ResLvl, Depth+1);
    else if (Split == ResolutionSplit)
      NParticlesRight += DecodeTreeIntDFSBlockBased(Mid, End, GridRight, NextSplit, ResLvl+1, Depth+1);
  }
  return N;
}
...
NaiveCompressUsingZfp(f64 Accuracy) {
  ...
  FOR(i32, D, 0, 3) {
    ...
    for (size_t I = 0; I < AllVals.size(); I += 64) {
      size_t J = I;
      for (; J < I + 64 && J < AllVals.size(); ++J) {
        BlockVals[J - I] = AllVals[J];
      }
      CompressBlockZfp(Accuracy, i32(J-I), BlockVals, &BlockStream);
    }
  }
}
    ...
    while (N) {
      ...
      if (N > 0) {
        ...
        FOR(int, I, 0, N) { // copy the data over
          ...
          Particles[C.ParticleId].Pos.z = f32(BlockFloats[I] + (I*1.0*W3.z));
  ...
  if (Params.NDims >= 2) {
    ...
    while (N) { // while there are still particles unprocessed
      ...
      if (N > 0) {
        ...
        FOR(int, I, 0, N) { // copy the data over
          const auto& C = ProjCells[I];
          Particles[C.ParticleId].Pos.y = f32(BlockFloats[I] + (I*1.0*W3.y));
  ...
  if (Params.NDims >= 1) {
    ...
    while (N) { // while there are still particles unprocessed
      ...
      if (N > 0) {
        ...
        FOR(int, I, 0, N) { // copy the data over
          const auto& C = ProjCells[I];
          Particles[C.ParticleId].Pos.x = f32(BlockFloats[I] + I*1.0*W3.x);
...
AllocateMemoryForBlocks(const grid_int& Grid) {
  ...
  FOR_EACH(S, Streams) {
    InitWrite(&S->Stream, 4 << 20); // 4 MB
    S->Coder.InitWrite(4 << 20);
  }
  InitWrite(&GlobalStream.Stream, 4 << 20); // 4 MB
  GlobalStream.Coder.InitWrite(4 << 20);
...
WriteBinaryFiles() {
  ...
  WriteVarByte(&Bs, Size(GlobalStream.Stream));
  WriteVarByte(&Bs, Size(GlobalStream.Coder.BitStream));
  StreamSize += Size(GlobalStream.Stream) + Size(GlobalStream.Coder.BitStream);
  FOR_EACH(S, Streams) {
    Flush(&S->Stream);
    S->Coder.EncodeFinalize();
    fwrite(S->Stream.Stream.Data, Size(S->Stream), 1, Fp);
    fwrite(S->Coder.BitStream.Stream.Data, Size(S->Coder.BitStream), 1, Fp);
    StreamSize += Size(S->Stream) + Size(S->Coder.BitStream);
    GrowToAccomodate(&Bs, 8);
    WriteVarByte(&Bs, Size(S->Stream));
    WriteVarByte(&Bs, Size(S->Coder.BitStream));
  }
  Flush(&Bs);
